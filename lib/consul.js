...
class Consul extends papi.Client {
  ...
  async snapshot() {
    const opts = utils.defaults({}, this._defaults);
    const req = {
      path: "/snapshot",
      query: {},
    };
    utils.options(req, opts);
    const list = await this._get(req);
    return list.body;
  }
  async restore(buf) {
    const opts = utils.defaults(opts, this._defaults);
    const req = {
      path: "/snapshot",
      query: {},
      type: "plain",
    ...
    utils.options(req, opts);
    await this.consul._put(req, utils.body);
  }
